...
def _configuration_values(root, image):
    ...
    return {
        'compose.env': {
            'INFRA_SYNC_COMPOSE_PROJECT': 'infra-sync', 'INFRA_SYNC_IMAGE': image,
            'INFRA_SYNC_CONFIG_DIR': str(root / 'config'),
...
def generate_configuration(root, image, *, destination=None):
    ...
    for name in CONFIG_NAMES:
        replacements = ({'INFRA_SYNC_IMAGE': image,
                         'INFRA_SYNC_CONFIG_DIR': str(destination)}
...
def _wait_for_postgres(root, release, config):
    ...
    for _attempt in range(60):
        status = run(compose_command(
            ...
            'infra_sync_bootstrap', '-d', 'infra_sync', release=release, config=config),
                     check=False)
...
def prepare_stack(prepared):
    ...
    run(compose_command(root, 'build', 'infra-sync-api', release=release, config=config))
...
def _runtime_services():
    return ('infra-sync-api', 'infra-sync-secret-broker', 'infra-sync-discovery-worker',
    ...
    for name in CONFIG_NAMES:
        ...
        if name == 'compose.env':
            payload = _merged_config(
                source, {}, {'INFRA_SYNC_CONFIG_DIR': str(prepared.root / 'config')})
        else:
...
def quiesce_uncertain_runtime(prepared):
    ...
    run(compose_command(
        prepared.root, 'stop', 'infra-sync-api', 'infra-sync-apply-worker',
        'infra-sync-schedule-worker', release=prepared.release,
        config=prepared.root / 'config'), check=False)


def _timer_active():
    return run(['systemctl', 'is-active', '--quiet', 'infra-netbox-sync.timer'],
...
def stop_timer():
    ...
    if was_active:
        run(['systemctl', 'stop', 'infra-netbox-sync.timer'])
    return was_active
...
def main(argv=None):
    ...
    try:
        ...
        try:
            ...
            lock_path = (Path('/run/infra-sync/apply.lock') if root == Path('/opt/infra-sync')
